//테스트케이스 1번의 시간초과의 원인을 알아내지못해서 블로그 확인
//블로그 내용
//DFS를 사용해서 값을 구하게된다면 시간초과가 발생하기때문에
//결과 값을 도출하는 수학적 공식을 사용해야한다.
//다항식과 관련된 수학적 공식을 도출한다.
//3개가 된다면? (a+b+c) + (ab+bc+ca) + (abc)가지
//즉 옷의 종류가 3가지고 각각의 옷의 개수가 a, b, c라면 (x+a)(x+b)(x+c) = x3 + (a+b+c)x2 + (ab+bc+ca)x + (abc)라는 식이 정립됩니다.총 조합의 개수가 계수에 다 포함되어 있습니다.
//우리가 원하는 식을 도출하기 위해서  x=1을 대입해주면 됩니다.
...
int solution(vector<vector<string>> clothes) {
   ...
   // DFS(0,0,vec);
    int result=1;
    //처음에 설명했던 것 처럼 x 자리에 1을 대입해서 곱하기때문에 result에 vec[i]+1 을 곱해준다.
